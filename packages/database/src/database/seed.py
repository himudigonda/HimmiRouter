...
async def seed_data():
    async with engine.begin() as conn:
        ...
        def sync_seed(session: SyncSession):
            # 1. Create Companies
            ...
            # 2. Create Providers
            direct_openai = Provider(name="OpenAI", website="https://api.openai.com")
            google_api = Provider(
                name="Google API", website="https://generativelanguage.googleapis.com"
            )
            session.add_all([direct_openai, google_api])
            session.flush()

            # 3. Create Models
            gpt4 = Model(name="GPT-4o", slug="openai/gpt-4o", company_id=openai_co.id)
            gemini = Model(
                name="Gemini 1.5 Pro",
